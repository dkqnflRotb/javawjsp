...
public class PdsTotalDownCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ServletOutputStream sos = null;  //클라이언트에 전송하기위해 필요함 줄81
		...
		for(int i=0; i<fNames.length; i++) {
			...
			// 업로드 한 파일을 복사하면서 이름을 바꾼다.
			File copyAndRename = new File(realPath + fNames[i]);
			
			file.renameTo(copyAndRename);   // copyAndRename 을 복사하면서 이름을 file로 바꾸는것이다. _a_renameTo(_b_) : b파일을 a파일로 이름바꿔서 복사처리한다.
			...
			zos.flush(); //나머지를 다 담아준다
			zos.closeEntry();  
			fis.close();
		}
		zos.close();  //서버에서 집파일 작업이 끝났다
