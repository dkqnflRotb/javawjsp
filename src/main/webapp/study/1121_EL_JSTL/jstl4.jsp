...
<div class="container">
  ...
  <p>
  	...
  	<hr/>
  		5-3.숙제 atom변수에 기억되어 있는 문자를 입력받아 그 문자가 두번째로 기억되어 있는 위치값을 출력하시오.
  	...
  	<form>
  		...
  		<c:set var="hwAtom" value="${param.str}"/>
  		<c:set var="cnt" value="0"/>
  		<c:forEach var="i" begin="0" end="${fn:length(atom)-1}">
	  		<c:if test="${fn:substring(atom,i,i+1) == hwAtom && cnt<=1}">
	  			<c:set var="cnt" value="${cnt+1}"/>
					<c:choose>
				 		<c:when test="${cnt == 1}"><c:set var="loca" value="${i}"/></c:when>
				 		<c:when test="${cnt == 2}"><c:set var="loca" value="${i}"/></c:when>
				 	</c:choose>  		
	  		</c:if>
  		</c:forEach> 
		  		<c:if test="${cnt==2}">"${hwAtom}"의 두번째 index값 = ${loca}<br/></c:if>
		  		<c:if test="${cnt==1}">"${hwAtom}"의 첫번째 index값 = ${loca} (두번째 값이 없습니다.)<br/></c:if>
  	</form>
  	<hr/>
  	</div>
  	<br/>	
  		...
  		6-2.숙제 atom변수에 기억되어 있는 문자를 입력받아 그 문자가 두번째로 기억되어 있는 위치값을 출력하시오.
  			 단, 두번째 기억값이 없으면 1번째 기억위치를 출력하고 '두번째 값 없음'을 함께 출력하시오.
  		<form name="myform2">
  		<input type="text" name="str2" value="${param.str2}"/>
  		<input type="submit" value="전송" class="btn btn-success"/><br/>
  		<c:set var="hwAtom2" value="${param.str2}"/>
  		<c:set var="afstr" value="${fn:substringBefore(atom,hwAtom2)}"/>
  		<c:set var="twostr" value="${fn:indexOf(afstr,hwAtom2)}"/>
  		<c:if test="${twostr<0}"> 첫번쨰 index값 = "${twostr}"</c:if>
  		
  		<%-- <c:if test="${twostr=>0}"></c:if> --%>
  		</form>
