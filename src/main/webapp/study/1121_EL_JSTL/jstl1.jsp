...
<div class="container">
  ...
  <h3>IF문(조건문 : 참일때만 수행할 내용이 있음. 즉ㅣ else문장이 없다.)</h3>
  ...
  <c:set var="su3" value=" 200"/>
 	...
 	3-2. su2 != su3 : <c:if test="${su2 != su3}">su2 과 su3는 다르다.(문자비교)</c:if><br/><br/>
 	<%-- 4-1. su2 == su3 : <c:if test="${su2+0 == su3+0}">su2 과 su3는 같다.</c:if><br/>
 	<!-- 숫자 비교할때 숫자를 먼저 더해주고 트림함수로 공백빼 주고 비교한다  -->
 	4-2. su2 != su3 : <c:if test="${su2+0 != su3+0}">su2 과 su3는 다르다.(문자비교)</c:if><br/><br/> --%>
 	<hr/>
 	<c:set var="jum" value="${param.jumsu}"/>
	<c:if test="${jum < 60 }"><c:set var="grade" value="F학점"/></c:if>
	<c:if test="${jum >= 60 }"><c:set var="grade" value="D학점"/></c:if>
	<c:if test="${jum >= 70 }"><c:set var="grade" value="C학점"/></c:if>
	<c:if test="${jum >= 80 }"><c:set var="grade" value="B학점"/></c:if>
	<c:if test="${jum >= 90 }"><c:set var="grade" value="A학점"/></c:if>
	<c:if test="${param.gender == 1 }"><c:set var="gen" value="남자"/></c:if>
	<c:if test="${param.gender == 2 }"><c:set var="gen" value="여자"/></c:if>
 	...
 	<form name="myform">
 		...
 		<div>
 			...
 			계산된 학점 :<font color="red"><b>${grade}</b></font><br/>
 			성별 : <font color="red"><b>${gen}</b></font>
 		</div>
 	</form>
 	<br/>
 	<hr/>
