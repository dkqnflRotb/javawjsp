...
<head>
  ...
  <script>
  	...
  	function fileBoxAppend(){
  		...
  		fileBox += '<input type="file" name="fName'+cnt+'" id= "fName'+cnt+'" class="form-control-file border mb-4" style="float:left;width:85%"/>';
  	...
  	function fCheck(){
  		...
  		for(let i=1; i<=cnt; i++){
  			...
  			if(fName != ""){
  				...
  				if(uExt != "JPG" && uExt != "PNG" && uExt != "ZIP" && uExt != "HWP" && uExt != "GIF" && uExt != "PPT" && uExt != "PPTX" && uExt != "FIFF" ){
  					alert("업로드 가능한 파일은 'JPG,PNG,ZIP,HWP,GIF,PPT,PPTX,FIFF'입니다.'");
  					return false;
  				}
  			}
  			if(fileSize > maxSize){
	...
	<div id="wrap">
		<div class="container">
			<form name ="myform" method="post" action="${ctp}/pdsInputOk.pds" enctype="multipart/form-data">
	  		...
	  		<br/>
				<div>
					<input type="button" value="파일박스추가" onclick="fileBoxAppend()" class="btn btn-success btn-sm mb-2"/>
	  			<input type="file" name="fName1" id= "fName1" class="form-control-file border mb-4"/>
				</div>	
				...
				<div class="mb-2">
					...
					<input type="button" value="돌아가기" onclick="location.href='${ctp}/pdsList.pds?part=${part}';" class="btn btn-info"/> &nbsp;  
				</div>
