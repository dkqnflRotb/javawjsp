      ...
      <td colspan="4" class="text-center">
        ...
        <c:if test="${flag != 'search' }">
	        ...
	        <c:if test="${sMid == vo.mid || sLevel ==0 }">
	        	<input type="button" value="수정하기" onclick="location.href='${ctp}/boUpdate.bo?idx=${vo.idx}&pageSize=${pageSize}&pag=${pag}';" class="btn btn-primary"/>
	  	      <input type="button" value="삭제하기" onclick="boDelCheck()" class="btn btn-danger"/> 
	        </c:if> <!--현재 게시판보는 것은 하나만있어서 boDelCheck()에 idx를 안보내도된다.   -->
        </c:if>
      </td>
    </tr>
  </table>
  	...
  	<table class="table text-center">
  		<tr>
  			...
  			<td style="width:15%">
  				...
  				<p>작성 : ${sNickName}</p>
  				<p>
  					<input type="button" value="댓글달기" onclick="replyCheck()" class="btn btn-primary btn-sm"/>
  				</p>
  			</td>
  		</tr>
  	</table>
