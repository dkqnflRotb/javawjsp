      ...
      // 전송전에 '주소'를 하나로 묶어서 전송처리 준비한다.
    	...
    	if(fName.trim() == ""){
    		...
    		if(fileSize > maxSize){
    			...
    			alert("업로드 가능한 파일에는 공백을 포함할수 없습니다.");
    			return false;
    		}
...
<div class="container">
  <form name="myform" method="post" action="${ctp}/memUpdateOk.mem" class="was-validated">
    ...
    <div class="form-group">
      ...
      <input type="text" value="${vo.nickName }"class="form-control" id="nickName" placeholder="비밀번호를 입력하세요." name="nickName" required />
    </div>
    <div class="form-group">
      <label for="name">성명 : &nbsp; &nbsp;
      <input type="text" value="${vo.name }" class="form-control" id="name" placeholder="별명을 입력하세요." name="name" required />
    	</label>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      <div class="form-check-inline">
        <span class="input-group-text">취미</span> &nbsp; &nbsp;
			...
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="낚시" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='낚시'}">checked</c:if></c:forEach>/>낚시
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="수영" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='수영'}">checked</c:if></c:forEach>/>수영
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="독서" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='독서'}">checked</c:if></c:forEach>/>독서
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="영화감상" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='영화감상'}">checked</c:if></c:forEach>/>영화감상
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="바둑" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='바둑'}">checked</c:if></c:forEach>/>바둑
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="축구" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='축구'}">checked</c:if></c:forEach>/>축구
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
			    <input type="checkbox" class="form-check-input" value="게임" name="hobby" <c:forEach var="hobby" items="${hobbys}" varStatus="st"><c:if test="${hobby=='게임'}">checked</c:if></c:forEach>/>게임
			  </label>
			</div>
			<div class="form-check-inline">
			  <label class="form-check-label">
